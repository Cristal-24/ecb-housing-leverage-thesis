...
# --------------------------------------------------
# 5. Create cumulative future change in log rents
# --------------------------------------------------
# y_{i,t,h} = 100 * [log(Rent_{i,t+h}) - log(Rent_{i,t})]
for h in horizons:
    df[f"lp_dlog_rent_h{h}"] = (
        df.groupby("country")["log_rent"].shift(-h) - df["log_rent"]
    ) * 100
...
for h in horizons:
    ...
    all_summaries.append(f"LOCAL PROJECTION - RENTS - HORIZON {h}")
...
lp_results.to_csv("lp_rents_results.csv", index=False)

with open("lp_rents_summaries.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(all_summaries))

# --------------------------------------------------
# 8. Print compact results
# --------------------------------------------------
print("\nLOCAL PROJECTION RESULTS - RENTS")
print(lp_results)
...
plt.title("Dynamic effect of ECB shock × leverage on rents")
plt.xlabel("Horizon (quarters ahead)")
plt.ylabel("Coefficient on shock × leverage")
plt.xticks(horizons)
plt.tight_layout()
plt.savefig("lp_rents_plot.png", dpi=300)
plt.show()
...
print("rent response in more leveraged countries.")

print("\nIf the coefficient is negative:")
print("- more leveraged countries experience a stronger decline")
print("  (or weaker cumulative growth) in rents after the shock.")

print("\nSaved files:")
print("- lp_rents_results.csv")
print("- lp_rents_summaries.txt")
print("- lp_rents_plot.png")
